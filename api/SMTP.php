...
function save_account() {
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'save_account') {
        ...
        // Validation
        if (empty($name) || empty($email) || empty($smtp_host) || empty($username) || empty($password)) {
        ...
        try {
            $stmt = $db->prepare("
                INSERT INTO accounts (id, name, email, smtp_host, smtp_port, username, password, use_ssl, from_name)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            ");
            
            $stmt->execute([$account_id, $name, $email, $smtp_host, $smtp_port, $username, $password, $use_ssl, $from_name]);
            $_SESSION['success'] = 'Account saved successfully! ✓';
        } catch (Exception $e) {
            $_SESSION['error'] = 'Error saving account: ' . $e->getMessage();
        }
    }
}
...
function send_email() {
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'send_email') {
        ...
        if (!$account_id) {
            $_SESSION['error'] = 'Please select an account';
            return;
        }
        ...
        try {
            ...
            // Message Data
            $mail->setFrom($account['email'], $account['from_name']);
...
<body>
    <div class="container">
        ...
        <div class="content">
            <div class="db-info">
                ✓ Database: SQLite (uses /tmp/smtp_mailer.db) - Works on any hosting!
            </div>
